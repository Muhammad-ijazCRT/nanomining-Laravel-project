...
function showCryptoAmount($amount, $decimal = 10, $separate = true, $exceptZeros = false)
{
    $separator = $separate ? ',' : '';
    $printAmount = number_format($amount, $decimal, '.', $separator);
...
function showCAmount($amount, $decimal = 2, $separate = true, $exceptZeros = false)
{
    $separator = '';
    if ($separate) {
        $separator = ',';
...
function levelCommission($user, $amount, $trx)
{
    ...
    while ($i <= $level) {
        ...
        $refer->referral_commission += $commissionValue;
        $refer->save();
