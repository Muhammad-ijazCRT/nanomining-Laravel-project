                ...
                <tr>
                    ...
                    <td>
                        <button class="btn--base btn--xsm viewBtn"
                            ...
                            data-status="{{ $data->status }}" data-order-id="{{ $data->id }}"
        ...
        <div class="modal-content rounded-0">
            ...
            <div class="modal-body">
                <ul class="list-group">
                    ...
                    <li class="list-group-item d-flex justify-content-between">
                        <span class="font-weight-bold">@lang('Mined Coin')</span>
                        <span class="plan-main-coin"></span>
                    </li>
                </ul>
            </div>
    ...
    <script>
        ...
        (function($) {
            $('.viewBtn').on('click', function() {
                ...
                function miningServerData() {
                    $.ajax({
                        url: "{{ route('user.single.mining.server.data') }}/" + data.orderId,
                        type: 'GET',
                        dataType: 'json',
                        headers: {
                        ...
                        success: function(data) {
                            ...
                            var btcAmount = data.mined_coins
                            var usdAmount = btcAmount * btcToUsdRate;

                            // console.log(usdAmount);

                            modal.find('.plan-main-coin').text(parseFloat(
                                    data.mined_coins).toFixed(
                                    10) +
                                ' ≈ $' + usdAmount.toFixed(2));
                        },
                ...
                intervalId = setInterval(miningServerData, 1000);

                modal.on('hidden.bs.modal', function() {
                    clearInterval(intervalId); // Clear the interval when modal is hidden
                })
