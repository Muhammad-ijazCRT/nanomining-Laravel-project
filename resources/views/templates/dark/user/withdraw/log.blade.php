        ...
        <form action="">
            <div class="input-group">
                <input class="form-control form--control" name="search" type="text" value="{{ request()->search }}"
                    placeholder="@lang('Transaction ID / wallet')">
            ...
            <tbody>
                @forelse($withdraws as $withdrawal)
                    <tr>
                        ...
                        <td>{{ __($withdrawal->userCoinBalance->wallet ?? 'USD') }}</td>
                        <td>
                            <strong>{{ showAmount($withdrawal->amount, 8, exceptZeros: true) }}
                                {{ strtoupper($withdrawal->userCoinBalance->miner->coin_code ?? 'USD') }}</strong>
                        </td>
                        ...
                        <td>
                            @if ($withdrawal->status == Status::PAYMENT_SUCCESS)
                                <button class="btn btn--success detailBtn btn--xsm"
                                    data-admin_feedback="{{ $withdrawal->admin_feedback }}"><i
                                        class="las la-desktop"></i></button>
                            @elseif($withdrawal->status == Status::PAYMENT_REJECT)
                                <button class="btn btn--danger detailBtn btn--xsm"
                                    data-admin_feedback="{{ $withdrawal->admin_feedback }}"><i
                                        class="las la-desktop"></i></button>
                            @else
                                <a class="btn btn--warning btn--xsm"
                                    href="{{ route('user.withdraw.preview', encrypt($withdrawal->id)) }}"><i
