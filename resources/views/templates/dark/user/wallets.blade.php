            ...
            <div class="wallet-card-wrapper">
                @foreach ($userCoinBalances as $item)
                    <div class="wallet-card">
                        <button class="edit-btn updateBtn" data-id="{{ $item->id }}"
                            data-title="{{ strtoupper($item->miner->coin_code) }}" data-wallet="{{ $item->wallet }}"><i
                                class="la la-pencil"></i></button>

                        <div class="wallet-card-body">
                            <div class="top">
                                <img alt="Image" class="logo"
                                    src="{{ getImage(getFilePath('miner') . '/' . $item->miner->coin_image, getFileSize('miner')) }}">
                                <div>
                                    <h5 class="title">{{ strtoupper($item->miner->coin_code) }} @lang('Wallet')</h5>
                                    <small>{{ showAmount($item->balance, 8, exceptZeros: true) }}</strong>
                                        {{ strtoupper($item->miner->coin_code) }}</small>
                                </div>
                            </div>
                    ...
                    <form action="" method="post">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input class="form--control" id="wallet" name="wallet" placeholder="@lang('Enter wallet Address')"
